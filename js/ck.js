...
/** CONTROLKIT (WORK IN PROGRESS)
 currently there's an error in one of the functions (see https://github.com/automat/controlkit.js/issues/58)
 so this GUI is not usable for the time being since other functions build off of it :( **/

let controlKit = new ControlKit();

let numberValue = {value:2, range: [1, 5]};

let colorValue = {color:'#ff00ff'};

controlKit.addPanel()
    .addGroup()
        .addSlider(numberValue,'value', 'range')
        .addSlider(numberValue,'value', 'range')
        .addSlider(numberValue,'value', 'range')
        .addColor(colorValue,'color',{colorMode:'hex'});
